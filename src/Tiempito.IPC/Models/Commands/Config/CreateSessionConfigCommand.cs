...
public sealed record CreateSessionConfigCommand : Command
{
    /// <summary>
    ...
    private const string DefaultDelayBetweenTimes = "0s";
    ...
    /// <param name="delayBetweenTimes">The delay between times, or null to use the default.</param>
    internal CreateSessionConfigCommand(
        Guid id,
        Guid correlationId,
        ...
        : base(id, correlationId, timestamp)
    {
        ...
        DelayBetweenTimes = delayBetweenTimes ?? DefaultDelayBetweenTimes;
    }

    /// <summary>
    /// Creates a new <see cref="CreateSessionConfigCommand"/> instance with the specified parameters.
    /// </summary>
    /// <param name="sessionConfigId">The unique identifier for the session configuration.</param>
    /// <param name="targetCycles">The target number of cycles for the session.</param>
    /// <param name="focusDuration">The duration of the focus period.</param>
    /// <param name="breakDuration">The duration of the break period.</param>
    /// <param name="delayBetweenTimes">The delay between times, or null to use the default.</param>
    /// <returns>A new <see cref="CreateSessionConfigCommand"/> instance with the provided parameters.</returns>
    public static CreateSessionConfigCommand CreateNew(
        string sessionConfigId,
        uint targetCycles,
        string focusDuration,
        string breakDuration,
        string? delayBetweenTimes)
    {
        ...
        ArgumentNullException.ThrowIfNull(targetCycles);
        ArgumentNullException.ThrowIfNull(focusDuration);
        ArgumentNullException.ThrowIfNull(breakDuration);

        return new CreateSessionConfigCommand(
            id: Guid.NewGuid(),
